...
// https://github.com/frankenbeanies/uuid4/
package gorand
...
func ParseString(str string) (uuid *UUID4, err error) {
	...
	if len(noDash) != 32 {
		return nil, errors.New(str + " is not a valid UUID4. The unhyphenated string representation should be 32 characters in length")
	}

	if noDash[12] != '4' {
		return nil, errors.New(str + " is not a valid UUID4. character 13 should be '4'.")
	}

	if noDash[16] != '8' && noDash[16] != '9' && noDash[16] != 'a' && noDash[16] != 'b' {
		return nil, errors.New(str + " is not a valid UUID4. character 17 should be '8', '9', 'a', or 'b'")
	}
